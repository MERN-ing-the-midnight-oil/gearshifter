...
export interface PrinterStatus {
  isConnected: boolean;
  currentDevice: PrinterDevice | null;
  isPrinting: boolean;
  error: string | null;
}
...
class PrinterService {
  ...
  async connect(device: PrinterDevice): Promise<boolean> {
    try {
      this.error = null;
      const success = await RNThermalPrinter.connectBluetooth(device.address);
  ...
  async disconnect(): Promise<void> {
    try {
      if (this.currentDevice) {
        await RNThermalPrinter.disconnectBluetooth();
   ...
   */
  private formatTagContent(options: PrintOptions): string {
   ...
   */
  private getFieldValue(item: Item, fieldName: string): string | number | null {
    switch (fieldName) {
      case 'item_number':
        return item.itemNumber;
      case 'description':
        return item.description || (item.customFields?.description as string) || '';
      case 'size':
        return item.size || (item.customFields?.size as string) || '';
      case 'original_price':
      ...
      case 'category':
        return item.category || (item.customFields?.category as string) || '';
      case 'status':
        return item.status;
      default:
        // Check custom fields
   ...
   */
  private formatFieldValue(value: string | number | null, format?: string): string {
    if (value === null || value === undefined) {
      return '';
    }
   ...
   */
  async printQRCodeAsImage(qrCodeImageBase64: string): Promise<boolean> {
    if (!this.currentDevice?.isConnected) {
      this.error = 'No printer connected';
      return false;
    }
    ...
    try {
      ...
      await RNThermalPrinter.printImageBase64(qrCodeImageBase64);
      return true;
    } catch (err) {
  ...
  async printTest(): Promise<boolean> {
    ...
    try {
      ...
      await RNThermalPrinter.printText(content);
      await RNThermalPrinter.cutPaper();
